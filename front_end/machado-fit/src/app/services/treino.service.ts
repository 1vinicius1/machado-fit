...
export class TreinoService {
  ...
  cadastrarTreino(cmd: CadastrarTreinoCmd): Observable<string> {
    return this.http.post(`${this.apiUrl}/treino/cadastrar`, cmd, { responseType: 'text' });
  }
  ...
  listarExerciciosPorAluno(idAluno: number): Observable<ExerciciosAlunoDTO[]> {
    return this.http.get<ExerciciosAlunoDTO[]>(`${this.apiUrl}/exercicio/listar-exercicios-aluno/${idAluno}`);
  }
  ...
  listarExerciciosPorTreino(idTreino: number): Observable<ExercicioDTO[]> {
    return this.http.get<ExercicioDTO[]>(`${this.apiUrl}/exercicio/listar-exercicios-treino/${idTreino}`);
  }
}